...
if [ "$?" -ne 0 ]; then
	mount -o compress=zstd:1 "$LOOP_DEV" "$MAIN_VOL_MOUNT_POINT"
else
...
mount --rbind /dev dev/
mkdir var/cache/apt/
mount --rbind /var/cache/apt/ var/cache/apt/
...
SIZE=$(btrfs filesystem usage -b .  | grep Free | grep min | cut -d: -f 3 | sed 's/)//' | xargs)
btrfs filesystem resize  -$SIZE .
# TODO resize the image file now
